        ...
        <table class="wp-list-table widefat fixed striped status-manager" id="statusManager">
            ...
            <tbody id="the-list">
                ...
                <tr id="status-<?php echo $slug; ?>"
                    ...
                    <td colspan="2"><span data-update-status-alias><?php echo isset( $aliases[ $status['alias'] ] ) ? $used_aliases[$status['alias']] = $aliases[ $status['alias'] ] : ''; ?></span></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
            <tfoot>
                    ...
                    <td>
                        <select name="status_alias">
                            ...
                            <?php foreach ( $aliases as $id => $name ) : ?>
                                <option value="<?php echo $id; ?>" <?php echo isset($used_aliases[$id])?'style="display: none"':'';?>><?php echo $name; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </td>
